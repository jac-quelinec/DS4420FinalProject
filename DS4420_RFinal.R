...
entropy_df <- recommend_df %>%
  group_by(State, Intervention) %>%
  summarise(TotalScore = sum(WeightedScore, na.rm = TRUE), .groups = "drop") %>%
  group_by(State) %>%
  mutate(Prob = TotalScore / sum(TotalScore)) %>%
  filter(!is.na(Prob) & Prob > 0) %>%  # ← KEY LINE
  summarise(Entropy = entropy::entropy(Prob, unit = "log2"))

head(entropy_df)

summary(entropy_df$Entropy)  
